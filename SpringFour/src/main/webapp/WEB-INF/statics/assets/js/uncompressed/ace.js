...
ace.enable_ajax_ = function($, options) {
	...
	.on('hashchange.ajax', function(e, manual_trigger) {
		...
		if(typeof _url === 'function') url = _url(hash);
	...
	function getUrl(url, hash, manual_trigger) {
	 ...
	 function updateTitle(link_text, contentArea) {
		...
		if($title.length > 0) {
			...
			var extra = $.trim(String(document.title).replace(/^(.*)[\-]/, ''));//for Controller like " - Ace Admin"
...
ace.sidebar_hoverable = function($, options) {
 ...
 //for Controller if "pulled up" in "responsive-min" mode, in "fullmode" should not remain "pulled up"
...
var is_element_pos =
 ...
 $sidebar.on('mouseenter.ace_hover', '.nav-list li.hsub', function (e) {
	...
	if(sub) {
		...
		if( ace.helper.hasClass(this, 'hover') && is_element_pos(sub, 'absolute') ) {//for Controller in small device .hover > .submenu may not be absolute anymore!
...
ace.settings_skin = function($) {
  ...
  $('#skin-colorpicker').on('change', function(){
	...
	//if(skin_class != 'skin-0') {
		...
		//for Controller: <body class="<?php echo $_COOKIE['ace_skin']; ?>"
...
/**
 The widget box reload button/event handler. You should use your own handler. An Controller is available at <i class="text-info">examples/widgets.html</i>.
