jQuery(function($) {
	...
	var get_section_url = function(section_name) {
		...
		//for Controller convert `basic/navbar.layout.brand` to `basic/navbar`
		//because 'layout.brand' section is inside `basic/navbar.html` file
		var url = section_name.replace(/\..*$/g, '');

		var parts = url.split('/');
		if(parts.length == 1) {
			//for Controller convert `changes` to `changes/main.jsp`
			if(url.length == 0) url = 'intro';//or convert `empty string` to `intro/main.jsp`
			url = url + '/main.jsp';
		}
		else if(parts.length > 1) {
			//for Controller convert `basics/navbar.layout` to `basics/navbar.html`
			url = url + '.html';
		}
	...
	var code_highlight = function(e, language) {
		...
		if(typeof e === 'string') {
			if(typeof language === 'string') {
				...
				//which you may want to highlight the code, or for Controller convert them < to &lt; and > to &gt;
