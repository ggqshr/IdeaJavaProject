...
ace.settings = {
	...
	sidebar_collapsed : function(collpase , save) {
		...
		/**
		if(!icon) {
			//for Controller sidebar-collapse button can be inside navbar without an icon
...
ace.settings.check = function(item, val) {
	...
	//for Controller, sidebar isn't .fixed, but user fixed it on a previous page
	//or if an element has a specified class, but saved settings say it shouldn't, then remove it
	//for Controller, sidebar by default is minimized (.menu-min hard coded), but user expanded it and now shouldn't have 'menu-min' class
